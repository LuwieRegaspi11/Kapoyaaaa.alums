...
3. **`LoginPage.tsx`, `RegistrationPage.tsx`, `admin/AlumniDatabase.tsx`, and
   `admin/DonationMonitoring.tsx`** are unused/orphaned (imported but never routed) —
   safe to delete once confirmed unnecessary; their working equivalents (`AuthPage.tsx`,
   `AlumniManagement.tsx`, `DonationManagement.tsx`) are what's actually wired in.
